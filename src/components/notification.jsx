...
const NotificationIcon = ({
  ...
  return (
    <div className="relative">
      ...
      {notificationCount > 0 && (
        <div
          className={`px-1 ${bgColor} rounded-full text-center text-white text-xs absolute top-[-6px] left-9`}
        >
          {notificationCount}
          <div
            className={`absolute top-0 left-0 rounded-full -z-10 animate-ping ${pingColor} w-full h-full`}
          ></div>
        </div>
      )}
      {showArticles && (
